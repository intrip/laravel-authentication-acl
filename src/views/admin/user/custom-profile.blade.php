...
@foreach($custom_profile->getAllTypesWithValues() as $profile_data)
    {{Form::open(["action" => "Jacopo\Authentication\Controllers\UserController@deleteCustomFieldType", 'class' => 'form-del-profile-field', 'role' => 'form'])}}
    <div class="form-group">
        <div class="input-group">
            <span class="input-group-addon form-button button-del-profile-field"><span class="glyphicon glyphicon-minus-sign add-input"></span></span>
...
<script>
    ...
    $(".button-del-profile-field").click( function(){
        if(confirm('Are you sure to delete this field?')) $('.form-del-profile-field').submit();
    });
</script>
@stop